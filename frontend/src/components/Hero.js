...
export const Hero = () => {
  ...
  return (
    <section data-testid="hero" className="relative overflow-hidden pt-[132px] pb-20 sm:pt-[168px] sm:pb-28">
      <div className="pointer-events-none absolute -left-40 -top-40 h-[620px] w-[620px] glow-blue" />
      <div className="pointer-events-none absolute -right-32 top-24 h-[520px] w-[520px] glow-cyan" />
      <div
      ...
      <div className="relative mx-auto grid max-w-[1240px] items-center gap-16 px-5 sm:px-8 lg:grid-cols-[1.15fr_0.85fr]">
        <div>
          <motion.div {...rise(0)} className="mb-7 inline-flex items-center gap-2.5 rounded-full border border-white/12 bg-white/[0.04] px-4 py-2">
            <span className="relative flex h-1.5 w-1.5">
              <span className="absolute inline-flex h-full w-full animate-ping rounded-full bg-[#4ade80]" />
              <span className="relative inline-flex h-1.5 w-1.5 rounded-full bg-[#4ade80]" />
            </span>
            <span className="text-[11px] font-bold uppercase tracking-[0.2em] text-white/70">{t.hero.badge}</span>
          </motion.div>

          <motion.h1 {...rise(0.08)} className="font-display text-5xl font-extrabold leading-[0.95] tracking-tighter sm:text-6xl lg:text-7xl">
            {t.hero.titleA}
            <br />
            <span className="bg-gradient-to-r from-[#2563eb] via-[#60d6ff] to-[#a8ecff] bg-clip-text text-transparent">{t.hero.titleB}</span>
          </motion.h1>

          <motion.p {...rise(0.16)} className="mt-7 max-w-xl text-base leading-relaxed text-white/55 lg:text-lg">
            {t.hero.sub}
          </motion.p>
          ...
          <motion.div {...rise(0.32)} className="mt-14">
            <p className="mb-4 text-[10px] font-bold uppercase tracking-[0.28em] text-white/35">{t.hero.platforms}</p>
            <div className="flex items-center gap-6">
              {PLATFORMS.map(({ name, Icon }) => (
                <span key={name} data-testid={`platform-${name.toLowerCase()}`} className="group flex items-center gap-2.5">
                  ...
                  <span className="hidden text-sm font-semibold text-white/40 transition-colors duration-300 group-hover:text-white sm:inline">{name}</span>
                </span>
              ))}
            </div>
          </motion.div>
        ...
        >
          ...
          <motion.img
            ...
            className="relative mx-auto w-full max-w-[380px] object-contain drop-shadow-[0_0_60px_rgba(37,99,235,0.45)]"
          />
        </motion.div>
      </div>
    </section>
